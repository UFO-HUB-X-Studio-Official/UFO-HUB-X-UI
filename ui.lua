...
-- ===== Player tab (Right) — MIN FACE (แยกส่วน, ไม่ทับของเก่า) =====
registerRight("Player", function(scroll)
    local BASE = rawget(_G, "THEME") or {}
    local THEME = {
        BG_INNER = BASE.BG_INNER or Color3.fromRGB(0, 0, 0),
    ...
    -- ให้ scroll มี layout กลาง ถ้ายังไม่มี
    local vlist = scroll:FindFirstChildOfClass("UIListLayout")
    if not vlist then
        vlist = Instance.new("UIListLayout")
        vlist.Padding = UDim.new(0, 12)
    ...
    -- ลบเฉพาะบล็อคของตัวเองก่อน (ถ้ามี) เพื่อไม่ซ้อนกัน
    local old = scroll:FindFirstChild("Section_MinFace")
    if old then old:Destroy() end

    -- ===== Section: Min Face =====
    local section = Instance.new("Frame")
    section.Name = "Section_MinFace"
    section.BackgroundTransparency = 1
    section.Size = UDim2.new(1, 0, 0, 0)
    section.AutomaticSize = Enum.AutomaticSize.Y
    section.LayoutOrder = 30  -- อยู่ถัดจากของเดิม
    section.Parent = scroll
    ...
    -- การ์ด “หน้าตาของมิน” (สไตล์อิโมจิ: วงกลมดำ ขอบเขียว)
    local card = Instance.new("Frame", section)
    card.BackgroundColor3 = THEME.BLACK
    card.Size = UDim2.fromOffset(180, 180)
    card.ClipsDescendants = false
    card.Parent = section
    -- มุมโค้งใหญ่ให้ฟีลเม็ดยา
    local c = Instance.new("UICorner", card); c.CornerRadius = UDim.new(0, 20)
    stroke(card, 1.8, THEME.GREEN)

    -- วงกลมหน้า (อิโมจิ) ตรงกลาง
    local faceWrap = Instance.new("Frame", card)
    faceWrap.AnchorPoint = Vector2.new(0.5, 0.5)
    faceWrap.Position = UDim2.fromScale(0.5, 0.46)
    faceWrap.Size = UDim2.fromOffset(110, 110)
    faceWrap.BackgroundColor3 = THEME.BLACK
    local cc = Instance.new("UICorner", faceWrap); cc.CornerRadius = UDim.new(1, 0) -- กลมเต็ม
    stroke(faceWrap, 1.8, THEME.GREEN)

    -- รูปภายใน (ใช้รูปเดิมได้—จะถูก “มองเป็นอิโมจิ” เพราะอยู่ในวงกลม)
    local img = Instance.new("ImageLabel", faceWrap)
    img.BackgroundTransparency = 1
    img.Size = UDim2.fromScale(0.84, 0.84)
    img.Position = UDim2.fromScale(0.08, 0.08)
    img.ScaleType = Enum.ScaleType.Fit
    -- ใส่ asset ของ “มิน” ได้ที่นี่ (ตัวอย่างใช้ภาพเดิม UFO ถ้ายังไม่มีรูปมิน)
    img.Image = "rbxassetid://89004973470552"

    -- ป้ายชื่อ “Min”
    local nameBar = Instance.new("Frame", card)
    nameBar.AnchorPoint = Vector2.new(0.5, 1)
    nameBar.Position = UDim2.fromScale(0.5, 1)
    nameBar.Size = UDim2.fromOffset(140, 34)
    nameBar.BackgroundColor3 = THEME.BLACK
    local cn = Instance.new("UICorner", nameBar); cn.CornerRadius = UDim.new(0, 10)
