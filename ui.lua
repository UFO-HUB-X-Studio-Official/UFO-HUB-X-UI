...
-- === ปุ่มสี่เหลี่ยมเล็กที่ปลายขวาแถบเลเวล ===
local toggleBtn = Instance.new("ImageButton", levelBar)
toggleBtn.Name = "OpenSidePanelButton"
toggleBtn.AutoButtonColor = false
toggleBtn.BackgroundColor3 = THEME.BG_INNER
toggleBtn.BorderSizePixel = 0
toggleBtn.Size = UDim2.fromOffset(24, 24)       -- ขนาดปุ่มเล็ก
toggleBtn.AnchorPoint = Vector2.new(1, 0.5)
toggleBtn.Position = UDim2.new(1, -6, 0.5, 0)   -- ชิดขวาในกรอบเลเวล
toggleBtn.Image = "rbxassetid://72289858646360"
corner(toggleBtn, 6)
stroke(toggleBtn, 1.2, THEME.GREEN, 0)

-- === แผง UI ด้านขวา (สีดำ ขอบเขียว) ===
-- NOTE: ต้องอาศัย RightShell ที่เราสร้างไว้ก่อนหน้า (ตัวกรอบด้านขวาใหญ่)
local sidePanel = Instance.new("Frame", RightShell)
sidePanel.Name = "PlayerSidePanel"
sidePanel.BackgroundColor3 = THEME.BG_INNER     -- พื้นดำธีมเดียวกัน
sidePanel.BorderSizePixel = 0
sidePanel.AnchorPoint = Vector2.new(0, 0.5)
sidePanel.Position = UDim2.new(1, 12, 0.5, 0)   -- โผล่ทางขวา “ตรงตำแหน่งสี่เหลี่ยมแดง”
sidePanel.Size = UDim2.fromOffset(240, 360)     -- ความกว้าง/สูงตามภาพตัวอย่าง
sidePanel.Visible = false                       -- เริ่มต้นซ่อน
sidePanel.ClipsDescendants = false
sidePanel.ZIndex = 50
corner(sidePanel, 10)
stroke(sidePanel, 1.6, THEME.GREEN, 0)

-- (ตัวอย่างของภายในแผง – เอาออก/แก้ได้)
do
    local lbl = Instance.new("TextLabel", sidePanel)
    lbl.BackgroundTransparency = 1
...
toggleBtn.MouseButton1Click:Connect(function()
    showPanel(not opened)
end)

-- (ออปชัน) ปิดแผงเมื่อสลับแท็บ/ซ่อนแท็บนี้
scroll.Parent:GetPropertyChangedSignal("Visible"):Connect(function()
    if not scroll.Parent.Visible then
        showPanel(false)
    end
end)
