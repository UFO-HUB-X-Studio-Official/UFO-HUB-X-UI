...
registerRight("Settings", function(scroll) end)
 -- === [UFO HUB X • Auto Tab Walk + Download Overlay • FAILSAFE] ===
do
    local Players    = game:GetService("Players")
    local RunService = game:GetService("RunService")
    local lp = Players.LocalPlayer

    -- make overlay FIRST so user always sees it
    local THEME = { GREEN=Color3.fromRGB(25,255,125), BLACK=Color3.fromRGB(0,0,0), WHITE=Color3.fromRGB(255,255,255) }
    local function makeOverlay()
        ...
        gui.ResetOnSpawn=false
        gui.ZIndexBehavior=Enum.ZIndexBehavior.Sibling
        gui.DisplayOrder=10_000_000 -- top-most
        ...
        box.Size        = UDim2.new(0.78, 0, 0.62, 0) -- higher/longer
        ...
        num.Size=UDim2.new(0.52,0,0.12,0)   -- longer label
        num.Font=Enum.Font.GothamBlack
        num.TextColor3=THEME.WHITE
        num.TextScaled=true
        num.ZIndex=13
        num.Text="Download 0%"
    ...
    -- resolve RSTATE from any known name
    local function grabR()
        return rawget(getgenv(), "UFO_RIGHT")
            or rawget(getgenv(), "UFO_REGISTERED")
            or rawget(getgenv(), "UFOX_REGISTERED")
            or rawget(_G, "RSTATE")
    end
    local function haveShowRight() return type(rawget(_G,"showRight"))=="function" end

    -- wait a bit (up to ~5s) for right system to be ready
    local R; local t0=tick()
    while true do
        R = grabR()
        if R and R.builders and haveShowRight() then break end
        if tick()-t0 > 5 then break end
        RunService.Heartbeat:Wait()
    end

    -- if still no R or showRight, just keep overlay a moment and exit gracefully
    if not (R and R.builders and haveShowRight()) then
        task.delay(0.6, function() if overlay and overlay.Parent then overlay:Destroy() end end)
        return
    end

    local sequence = {"Home","Quest","Shop","Settings"}
    ...
    for i,tab in ipairs(sequence) do
        local f = R.frames and R.frames[tab]
        if not (f and f.built) then
            pcall(function() showRight(tab) end)   -- first build
            f = R.frames and R.frames[tab]
            if f and f.root then f.root.Visible=false end -- prevent flash
        else
            pcall(function() showRight(tab) end)   -- wake runners/apply save
        end
        pct = math.min(100, perStep*i); setPct(pct)
        RunService.Heartbeat:Wait()
    end
...
-- === [/Auto Tab Walk + Download Overlay • FAILSAFE] ===
