...
-- ===== UFO HUB X • Player • AFK 💤 (Model A V1 • Right, #4, switch-only) =====
-- 1) โหมดหน้าจอมืด (เต็มจอ)
-- 2) หน้าจอข่าว (เต็มจอ)
-- 3) กันโดนเตะ 20 นาที (Anti-Idle)
-- 4) ตรวจจับไม่ขยับ 10 นาที -> กระตุ้นกันเตะ
-- NOTE: อยู่แท็บ Player (Right) และแสดงเป็น “อันที่ 4” ใต้ระบบวิ่ง/กระโดด
...
registerRight("Player", function(scroll)
    ...
    section.Size=UDim2.new(1,0,0,0)
    section.AutomaticSize=Enum.AutomaticSize.Y
    section.LayoutOrder = 4000           -- << ดันเป็นอันที่ 4 ล่างชุดเดิม
    section.Parent = scroll
    ...
    -- ======= สร้างแถว + สวิตช์ (สไตล์เดียวกับอันอื่น) =======
    ...
    -- ======= Overlay (สร้างเมื่อเปิดจริง เพื่อลดบั๊ก) =======
    ...
    createSwitchRow(section, "AFK กันโดนเตะ (20 นาที)", S.antiIdleOn, function(v)
        S.antiIdleOn = v
