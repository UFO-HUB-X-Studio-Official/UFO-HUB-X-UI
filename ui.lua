...
--===== UFO HUB X • Shop — MAX 🛸 (A V1 • row EXACT like Settings) =====
registerRight("Shop", function(scroll)
    ...
    if not scroll:FindFirstChild("MAX_Header") then
        ...
        head.Font=Enum.Font.GothamBold; head.TextSize=16; head.TextColor3=THEME.TEXT
        head.TextXAlignment=Enum.TextXAlignment.Left; head.Text="MAX 🛸"
        head.LayoutOrder = 10
    end

    -- Row: EXACT same geometry as Settings rows
    local row = scroll:FindFirstChild("MAX_Row1")
    if not row then
        row = Instance.new("Frame", scroll)
        row.Name="MAX_Row1"
        row.Size=UDim2.new(1,-6,0,46)             -- <- 46px like A V1 Settings
        row.BackgroundColor3=THEME.BLACK
        row.LayoutOrder=11
        corner(row,12); stroke(row,2.2,THEME.GREEN)

        -- Left label (same margins as Settings)
        local lab=Instance.new("TextLabel", row)
        lab.BackgroundTransparency=1
        lab.Position=UDim2.new(0,16,0,0)
        lab.Size=UDim2.new(1,-(16+12+220+12),1,0) -- กินพื้นที่ซ้ายทั้งหมด หักพื้นที่ปุ่มขวา
        lab.Font=Enum.Font.GothamBold; lab.TextSize=13; lab.TextColor3=THEME.WHITE
        lab.TextXAlignment=Enum.TextXAlignment.Left; lab.Text="MAX"

        -- Right button (exact 220x24, right inset 12px)
        local BTN_W, BTN_H, RIGHT = 220, 24, 12
        ...
        input.Position=UDim2.new(1, -(RIGHT+BTN_W), 0.5, -BTN_H/2)
        ...
        -- ถ้ามีพาเนลค้นหาที่ทำไว้แล้ว ให้สลับแสดง/ซ่อน (ไม่สร้างใหม่)
