...
registerRight("Settings", function(scroll)
    ...
    ----------------------------------------------------------------------
    -- เพิ่มระบบ Autorun ตอนรัน UI หลัก (เหมือนเดิม 100% แต่เปิดอัตโนมัติถ้าเซฟไว้)
    ----------------------------------------------------------------------
    task.defer(function()
        repeat task.wait(0.05) until game:GetService("CoreGui"):FindFirstChild("UFO_HUB_X_UI")
        if savedOn and not (S.frame and S.frame.Parent) then
            createFPSFrame()
            setSwitch(true)
        end
    end)
