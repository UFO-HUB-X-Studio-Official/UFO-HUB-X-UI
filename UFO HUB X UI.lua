...
-- === UFO: Left Overlay (ไม่แตะของเดิม, วางทับฝั่งซ้ายอย่างเดียว) ===
local Players = game:GetService("Players")
local RunS    = game:GetService("RunService")
local LP      = Players.LocalPlayer

-- 1) หา GUI/Left โดยไม่พังของเดิม
local pg   = LP:WaitForChild("PlayerGui")
local gui  = pg:FindFirstChild("UFOHubX") or pg:FindFirstChildOfClass("ScreenGui")
local window = gui and gui:FindFirstChild("Window", true)
local Left   = window and (window:FindFirstChild("LeftPanel", true) or window:FindFirstChild("Left", true))

if not Left then
    warn("[UFO] ไม่พบ Left/LeftPanel เลยยังวางการ์ดไม่ได้")
    return
end

-- 2) สร้างการ์ดซ้อนทับ (ถ้ามีอยู่แล้วจะไม่ซ้ำ)
local Card = Left:FindFirstChild("UFO_LeftCard")
if not Card then
    Card = Instance.new("Frame")
    Card.Name = "UFO_LeftCard"
    Card.Parent = Left
    Card.BackgroundTransparency = 1
    Card.ZIndex = 999                                -- สูงพอเห็นชัด แต่ไม่ยุ่งของเดิม
    -- ปรับตำแหน่งง่ายๆ: X,Y (เลื่อนซ้ายขวา/ขึ้นลงที่สองค่านี้)
    Card.Position = UDim2.new(0, 8, 0, 56)          -- X=8, Y=56
    -- ปรับความกว้าง/สูงของพื้นที่การ์ด
    Card.Size     = UDim2.new(1, -16, 0, 220)       -- เต็มความกว้างซ้าย, สูง 220px
end
...
Avatar.Position = UDim2.new(0, 0, 0, 0)             -- มุมซ้ายบนของ Card
Avatar.Size     = UDim2.fromOffset(120, 120)        -- ปรับขนาดรูปได้ตรงนี้
Avatar.ZIndex   = 1000
do
...
-- 4) ชื่อผู้เล่น
local Name = Card:FindFirstChild("Name") or Instance.new("TextLabel", Card)
Name.Name = "Name"
Name.BackgroundTransparency = 1
Name.Position = UDim2.new(0, 0, 0, 128)             -- ใต้รูป
...
-- 5) เวลาที่ใช้ UI (นับเฉพาะตอนสคริปต์นี้กำลังรันอยู่)
local Time = Card:FindFirstChild("Time") or Instance.new("TextLabel", Card)
Time.Name = "Time"
Time.BackgroundTransparency = 1
Time.Position = UDim2.new(0, 0, 0, 154)             -- ใต้ชื่อ
...
-- 6) ตัวนับเวลา + เปลี่ยนสีชื่อ (7วัน=เขียว, 30วัน=ทอง, 1ปี=แดง)
local start = os.time()
local acc = 0
RunS.Heartbeat:Connect(function(dt)
    acc += dt; if acc < 1 then return end; acc = 0
    local total = os.time() - start
    local d = math.floor(total/86400)
    local h = math.floor((total%86400)/3600)
    local m = math.floor((total%3600)/60)
    Time.Text = string.format("ใช้ UFO HUB X: %d วัน %d ชม. %d นาที", d, h, m)

    if     d >= 365 then Name.TextColor3 = Color3.fromRGB(255,60,60)
    elseif d >= 30  then Name.TextColor3 = Color3.fromRGB(255,215,0)
    elseif d >= 7   then Name.TextColor3 = Color3.fromRGB(0,255,140)
    else                 Name.TextColor3 = Color3.fromRGB(255,255,255)
    end
end)
-- === END UFO: Left Overlay ====================================================
